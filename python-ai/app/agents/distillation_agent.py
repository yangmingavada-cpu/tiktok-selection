...
description 用于未来的相关性筛选，必须包含足够具体的信息。

好的 description（包含市场/品类/策略等具体关键词）：
- "泰区家居$5-50综合评分策略"
- "美区宠物用品日榜筛选偏好"
- "不要自动添加高销量门槛"
- "用户习惯先筛后评的构建顺序"

差的 description（过于泛化，无法用于相关性筛选）：
- "选品相关记忆"
- "用户偏好设置"
- "一次选品的参数"
- "AI行为纠正"

规则：description 必须至少包含一个具体实体（市场名、品类名、价格范围、具体策略名、具体工具名）或一个具体行为描述。
...
def _is_valid_memory_candidate(item: object) -> bool:
    ...
    if name in generic_names:
        return False
    # 泛化描述黑名单：过于笼统的 description 无法用于语义召回筛选
    generic_descriptions = {"选品相关", "用户偏好", "项目信息", "会话记录", "参考信息", "行为反馈", "AI行为"}
    if any(description == g or description.startswith(g + "的") or description.startswith(g + "设置") for g in generic_descriptions):
        return False
