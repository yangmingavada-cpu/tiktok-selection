...
/**
 * DS07 - 视频列表筛选 (Video List Filter).
 * Queries the Echotik video/list endpoint with region, sales type,
 * ad flag, AI flag, time/duration range and sort options, supporting pagination.
 *
 * @author system
 * @date 2026/03/22
 */
@Component
public class VideoListFilterExecutor implements BlockExecutor {

    private static final Logger log = LoggerFactory.getLogger(VideoListFilterExecutor.class);
    private static final McpBlock BLOCK_META = VideoListFilterRequest.class.getAnnotation(McpBlock.class);

    private final EchotikApiClient echotikApiClient;

    public VideoListFilterExecutor(EchotikApiClient echotikApiClient) {
    ...
    public BlockResult execute(BlockContext context) throws Exception {
        ...
        try {
            VideoListFilterRequest req = VideoListFilterRequest.from(context.getBlockConfig());
            ...
            BlockResult result = BlockResult.success(allData, new ArrayList<>(VideoListFilterRequest.OUTPUT_FIELDS),
