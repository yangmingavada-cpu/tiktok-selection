...
/**
 * DS05 - 店铺列表筛选 (Seller List Filter).
 * Queries the Echotik seller/list endpoint with region, category,
 * shop type, sales mode and trend filters, supporting pagination.
 *
 * @author system
 * @date 2026/03/24
 */
@Component
public class SellerListFilterExecutor implements BlockExecutor {

    private static final Logger log = LoggerFactory.getLogger(SellerListFilterExecutor.class);
    private static final McpBlock BLOCK_META = SellerListFilterRequest.class.getAnnotation(McpBlock.class);

    private final EchotikApiClient echotikApiClient;

    public SellerListFilterExecutor(EchotikApiClient echotikApiClient) {
    ...
    public BlockResult execute(BlockContext context) throws Exception {
        ...
        try {
            SellerListFilterRequest req = SellerListFilterRequest.from(context.getBlockConfig());
            ...
            BlockResult result = BlockResult.success(allData, new ArrayList<>(SellerListFilterRequest.OUTPUT_FIELDS),
