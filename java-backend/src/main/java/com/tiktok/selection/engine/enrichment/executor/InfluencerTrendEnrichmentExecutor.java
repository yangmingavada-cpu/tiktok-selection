...
/**
 * EN04 - 达人历史趋势补充 (Influencer Historical Trend Enrichment).
 * 对每个达人调用 influencer/trend，分页获取日期区间内的趋势数据，
 * 将所有记录合并为 trend_data 列表追加到达人记录中。
 *
 * @author system
 * @date 2026/03/24
 */
@Component
public class InfluencerTrendEnrichmentExecutor implements BlockExecutor {

    private static final Logger log = LoggerFactory.getLogger(InfluencerTrendEnrichmentExecutor.class);
    private static final McpBlock BLOCK_META = InfluencerTrendEnrichmentRequest.class.getAnnotation(McpBlock.class);
    private static final DateTimeFormatter DATE_FMT = DateTimeFormatter.ofPattern("yyyy-MM-dd");
    private static final String ID_FIELD = "influencer_id";

    private final EchotikApiClient echotikApiClient;

    public InfluencerTrendEnrichmentExecutor(EchotikApiClient echotikApiClient) {
    ...
    public BlockResult execute(BlockContext context) throws Exception {
        ...
        InfluencerTrendEnrichmentRequest req = InfluencerTrendEnrichmentRequest.from(context.getBlockConfig());
        ...
        try {
            for (Map<String, Object> item : enriched) {
                ...
                while (trendRecords.size() < maxRecords) {
                    ...
                    params.put("user_id", entityId.toString()); // API 参数名为 user_id
                ...
                item.putIfAbsent("fans_growth_trend",
                    trendRecords.isEmpty() ? null : trendRecords);
            }
    ...
    private static List<String> buildOutputFields(List<String> availableFields) {
        ...
        for (String extra : InfluencerTrendEnrichmentRequest.EXTRA_FIELDS) {
