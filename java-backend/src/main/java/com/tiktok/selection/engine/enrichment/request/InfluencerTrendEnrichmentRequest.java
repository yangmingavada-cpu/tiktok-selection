...
@McpBlock(
    ...
    endpoint = "influencer/trend",
    outputType = "influencer_list",
    description = "达人历史趋势补充"
)
public class InfluencerTrendEnrichmentRequest {

    private static final ObjectMapper MAPPER = new ObjectMapper()
        .configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);

    public static final List<String> EXTRA_FIELDS = List.of(
        "trend_data", "fans_growth_trend"
    );
    ...
    @McpParam(desc = "每个达人最多获取趋势记录总数", type = "integer", defaultValue = "100")
    public Integer max_records = 100;

    public static InfluencerTrendEnrichmentRequest from(Map<String, Object> config) {
        return MAPPER.convertValue(config, InfluencerTrendEnrichmentRequest.class);
    }
