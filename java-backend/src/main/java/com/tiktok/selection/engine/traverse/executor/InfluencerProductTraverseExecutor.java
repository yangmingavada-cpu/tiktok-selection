...
/**
 * TR06 - 达人→商品 (Influencer to Product Traverse).
 *
 * @author system
 * @date 2026/03/24
 */
@Component
public class InfluencerProductTraverseExecutor implements BlockExecutor {

    private static final Logger log = LoggerFactory.getLogger(InfluencerProductTraverseExecutor.class);
    private static final McpBlock BLOCK_META = InfluencerProductTraverseRequest.class.getAnnotation(McpBlock.class);
    private static final String INPUT_ID_FIELD = "influencer_id";

    private final EchotikApiClient echotikApiClient;

    public InfluencerProductTraverseExecutor(EchotikApiClient echotikApiClient) {
    ...
    public BlockResult execute(BlockContext context) throws Exception {
        ...
        if (inputData == null || inputData.isEmpty()) {
            ...
            BlockResult result = BlockResult.success(Collections.emptyList(),
                new ArrayList<>(InfluencerProductTraverseRequest.OUTPUT_FIELDS), BLOCK_META.outputType(), 0, 0, 0L);
            result.setEchotikApiCalls(0);
            result.setApiEndpointsCalled(List.of(BLOCK_META.endpoint()));
            return result;
        }

        InfluencerProductTraverseRequest req = InfluencerProductTraverseRequest.from(context.getBlockConfig());
        int maxItems = req.getEffectiveMaxItems();
        int pageSize = req.getEffectivePageSize();
        int maxProductsPerInfluencer = req.getEffectiveMaxProductsPerInfluencer();
        ...
        try {
            for (Map<String, Object> item : itemsToProcess) {
                ...
                while (fetchedCount < maxProductsPerInfluencer) {
                    Map<String, Object> params = new HashMap<>();
                    params.put("user_id", entityId.toString());
                    params.put("page_num", pageNum);
                    params.put("page_size", pageSize);
                    if (req.category_id != null) {
                        params.put("category_id", req.category_id);
                    }
                    if (req.influencer_product_sort_field != null) {
                        params.put("influencer_product_sort_field", req.influencer_product_sort_field);
                    }
            ...
            BlockResult result = BlockResult.success(allResults,
                new ArrayList<>(InfluencerProductTraverseRequest.OUTPUT_FIELDS),
