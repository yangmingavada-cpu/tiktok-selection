...
@McpBlock(
    blockId = "SOURCE_PRODUCT_RANKLIST",
    endpoint = "product/ranklist",
    outputType = "product_list",
    description = "商品榜单"
)
@SuppressWarnings("java:S116")
public class ProductRanklistRequest {
    ...
    @McpParam(desc = "榜单日期，yyyy-MM-dd格式", required = true, example = "2026-03-25")
    public String date;
    ...
    @McpParam(desc = "榜单排序字段，1=销量榜 2=达人带货榜", required = true, type = "integer",
        enumValues = {"1", "2"})
    public Integer product_rank_field;

    @McpParam(desc = "榜单周期，1=天榜 2=周榜 3=月榜", required = true, type = "integer",
        enumValues = {"1", "2", "3"})
    public Integer rank_type;

    @McpParam(desc = "商品一级类目ID")
    public String category_id;

    @McpParam(desc = "商品二级类目ID")
    public String category_l2_id;

    @McpParam(desc = "商品三级类目ID")
    ...
    public static ProductRanklistRequest from(Map<String, Object> config) {
        return MAPPER.convertValue(config, ProductRanklistRequest.class);
    }
    ...
    public Map<String, Object> toBaseApiParams() {
        ...
        if (product_rank_field != null) params.put("product_rank_field", product_rank_field);
