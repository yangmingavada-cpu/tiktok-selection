...
-- 兼容迁移：对话历史 / 选品记录 独立隐藏与独立标题
-- 两个 flag 完全独立，不联动 delete_time。允许同一行在两个视图各自被 hide 或重命名
ALTER TABLE db_session.session ADD COLUMN IF NOT EXISTS hidden_from_chat BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE db_session.session ADD COLUMN IF NOT EXISTS hidden_from_records BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE db_session.session ADD COLUMN IF NOT EXISTS chat_title VARCHAR(255) DEFAULT NULL;
COMMENT ON COLUMN db_session.session.hidden_from_chat IS '是否从"对话历史"侧栏隐藏（不影响选品记录页）';
COMMENT ON COLUMN db_session.session.hidden_from_records IS '是否从"选品记录"页隐藏（不影响对话历史侧栏）';
COMMENT ON COLUMN db_session.session.chat_title IS '对话历史独立标题，NULL 时 fallback 到 title';
